...
class TestDepBlockedTagging(unittest.TestCase):
    ...
    494-500). The claim-gate message must flag dep-blocked tasks
    so the agent knows option (a) [claim] is a guaranteed-fail
    loop and only option (b) [cancel] is available."""

    def tearDown(self):
        _reset_event_loop()

    def test_dep_blocked_task_is_tagged_in_message(self):
        with tempfile.TemporaryDirectory() as tmp:
            reg = HubRegistry(Path(tmp), project_id="p", project_name="P")
            agent = _StubAgent("backend", reg)
            ...
            # PR 2.5-fix-2 (reviewer follow-up): tighten assertion
            # from ``assertIsNotNone`` to the exact gate shape so a
            # regression that returned ``{"action": "halt"}`` or
            # ``{"action": "finish"}`` (which would let finish
            # through and defeat the gate) fails the test.
            self.assertEqual(outcome, {"action": "continue"},
                              "gate must return the 'continue' "
                              "action — anything else lets finish "
                              "through and defeats the policy.")
            blob = " ".join(getattr(m, "content", "") or "" for m in msgs)
            self.assertIn("dep-blocked", blob,
                           "dep-blocked task was not tagged in the "
                           "rejection message — agent will try to "
                           "claim a task that claim_task refuses, "
                           "guaranteed retry loop until retry cap.")
            self.assertIn(child["id"], blob)
    ...
    def test_actionable_and_dep_blocked_counts_in_message(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            blob = " ".join(getattr(m, "content", "") or "" for m in msgs)
            self.assertIn("1 dep-blocked", blob)
            self.assertIn("1 claimable", blob)
