...
class DeliverProjectHappyPathTests(unittest.TestCase):
    def setUp(self) -> None:
        import os
        # This suite tests the in-tool happy path (checklist + retro policy), NOT the live
        # deliverability gate (DeliverProjectTool GUARD 2, default-on since fix #20). The agent
        # is a MagicMock, so the live gate would read an empty/mock hub and block — disable it.
