...
class OrchestratorSubscribesToMeetingDecisionAdded(unittest.TestCase):
    """Invariant 3: orchestrator's default subscriptions include the
    workhub.meeting_decision_added trigger that wakes its tick for
    synthesis attempts."""

    def test_orchestrator_default_subscriptions_include_meeting_decision_added(
        self,
    ) -> None:
        subs = DEFAULT_SUBSCRIPTIONS.get("orchestrator") or []
        self.assertIn(
            ("workhub", "meeting_decision_added", "normal"),
            subs,
            "orchestrator lane MUST subscribe to "
            "workhub.meeting_decision_added so the kickoff synthesis "
            "tick wakes on attendee decisions. Live subs: "
            f"{subs}",
        )
    ...
    def test_kickoff_complete_wired_for_attendees_plus_observers(self) -> None:
        """kickoff_complete wakes the four M1 attendees plus the
        debugger + knowledge observers — six lanes total."""
        expected_observers = set(KICKOFF_ATTENDEES) | {"debugger", "knowledge"}
...
class OrchestratorPromptCarriesKickoffSynthesisMacro(unittest.TestCase):
    """The orchestrator_agent.j2 file must define
    ``kickoff_synthesis_prompt`` so the lane's tick prompt is
    rendered when waking on meeting_decision_added events."""
    ...
    def test_kickoff_synthesis_prompt_macro_defined(self) -> None:
        ...
        self.assertTrue(
            re.search(
                r"\{%\s*macro\s+kickoff_synthesis_prompt\s*\(",
                src,
            ),
            "orchestrator_agent.j2 must define a "
            "``kickoff_synthesis_prompt(meeting_id, milestone_index=...)`` "
            "macro — the LLM lane wakes on meeting_decision_added and "
            "needs this prompt to drive try_synthesize/finalize_kickoff.",
        )
