...
class NonContractKeysTests(unittest.TestCase):
    ...
    def test_frontend_mixed_aux_and_auth_flags_only_auth(self):
        # done_def is a legitimate aux kickoff field (frontend prompt FINAL PROTOCOL
        # step 4); only the framework-owned auth_model is wrong-keys now — the message
        # names exactly the offender instead of looping the whole blob (run bsb900gpt).
        self.assertEqual(
            non_contract_keys({"auth_model": "jwt", "done_def": ["x"]}, "frontend"),
            ["auth_model"])

    def test_aux_only_decision_is_accepted(self):
        # an aux-only decision (done_def / feature_inventory*) must NOT be flagged →
        # no 18× resend loop; the reconcile reads these fields.
        self.assertEqual(non_contract_keys({"done_def": ["x"]}, "frontend"), [])
        self.assertEqual(
            non_contract_keys({"feature_inventory_auth": "...", "done_def": ["x"]}, "frontend"), [])
