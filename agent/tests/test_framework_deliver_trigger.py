...
def _orch(gate_failed_checks, *, implemented=True):
    ...
    # Deliberately NO output_dir: `ensure_fresh_smoke_before_cut` returns True when
    # there is none, and giving it an empty temp dir instead ARMS that gate, which
    # then holds the release on a cold-start re-validation this file is not about.
    # The TEST-USER SQUAD defers the FIRST delivery tick to run in the background
    # ("launched in BACKGROUND (single-flight, 0s deferred) — deferring this
    # delivery tick"), which is a different mechanism from the gate this file is
    # about. Mark it already-passed so the deliver trigger is what gets tested.
