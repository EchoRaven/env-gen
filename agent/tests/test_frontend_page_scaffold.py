...
class ScaffoldPagesFromContractTests(unittest.TestCase):
    def test_projects_stub_per_page_and_wires_routes(self):
        ...
        # 3 declared + framework-owned /login + /signup auth pages
        self.assertEqual(rep["routes"], 5)
        self.assertTrue(rep["app_wired"])
        built = sorted(os.listdir(Path(d) / "src" / "pages"))
        self.assertEqual(built, ["HomePage.jsx", "LoginPage.jsx", "SignupPage.jsx",
                                 "YoutubeShorts.jsx", "YoutubeWatch.jsx"])
        app = (Path(d) / "src" / "App.jsx").read_text()
        self.assertIn("BrowserRouter", app)
        self.assertEqual(app.count('<Route '), 5)  # 3 page + 2 auth routes
    ...
    def test_regenerates_nonrouter_stub_app_but_preserves_pages(self):
        # PROPOSAL #39 G1: a lane App.jsx that dropped the marker AND has no router
        # (no </Routes>) is a BROKEN stub (renders blank) — regenerate it (wiring the
        # declared pages) rather than preserve it. The lane's real PAGE files are NOT
        # touched (only App.jsx is rewritten).
        d = _mk_src()
        ...
        self.assertNotIn("src/pages/HomePage.jsx", rep["scaffolded"])  # lane page untouched
        self.assertTrue(rep["app_wired"])          # non-router stub regenerated
        app = (Path(d) / "src" / "App.jsx").read_text()
        self.assertIn("BrowserRouter", app)
        self.assertIn("HomePage", app)            # declared page now routed
        self.assertIn("REAL lane page",
                      (Path(d) / "src" / "pages" / "HomePage.jsx").read_text())

    def test_preserves_lane_app_with_real_router(self):
        # A genuine lane router (HAS </Routes>) is preserved — only additive route
        # injection applies, never a clobber.
        d = _mk_src()
        ...
        (Path(d) / "src" / "App.jsx").write_text(lane_app)
        (Path(d) / "src" / "pages").mkdir()
        (Path(d) / "src" / "pages" / "HomePage.jsx").write_text("// REAL lane page\n")
        rep = scaffold_pages_from_contract(
            d, [{"name": "home", "route": "/", "component": "HomePage"}])
        self.assertFalse(rep["app_wired"])         # real router preserved (additive path)
        self.assertIn("REAL lane page",
                      (Path(d) / "src" / "pages" / "HomePage.jsx").read_text())
    ...
    def test_idempotent(self):
        ...
        # the declared page isn't re-written; only the framework auth pages are
        # (always overwritten — the lane keeps shipping broken login).
        self.assertNotIn("src/pages/Home.jsx", rep2["scaffolded"])
    ...
    def test_dedups_duplicate_routes(self):
        ...
        scaffold_pages_from_contract(d, pages)
        app = (Path(d) / "src" / "App.jsx").read_text()
        # both pages routed, but not to the SAME path (+ 2 framework auth routes)
        self.assertEqual(app.count('<Route '), 4)
