...
# #1202sx: one helper for the readers that used to outlive their tests.
...
class TestGlobalEventsEndpoint(unittest.TestCase):
    ...
    def test_global_events_endpoint_streams(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            t.start()
            try:
                # #1202sx: this reader had `except Exception: break` -- no stray
                # TimeoutError, but a REAL fault vanished the same way, and its socket
                # (timeout=5) still outlived the 1.5s window and the join(2.0). The shared
                # helper bounds the socket below the window and hands faults back.
                received, meta, errors, rt = read_stream_1202sx(
                    port, "/api/events", 1.5, chunk=256)
                time.sleep(0.3)
