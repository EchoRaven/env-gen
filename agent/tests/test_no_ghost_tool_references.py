...
says call ``registryhub_register_ui_page``, no such tool"). One test, one fence.
...
KNOWN_MACROS: Set[str] = {
    ...
    "design_analyst_system_prompt", "design_analyst_task_prompt",  # design_analyst.j2 macros
    "design_system",  # the design doc (design_system.json / JSON key), not a tool
    # #1202bv helpers in runtime/design_prep.py — plain predicates over the design doc,
    # not tools. `design_system_is_enriched` had existed for a long time but was only ever
    # CALLED from orchestrator.py (outside the scanned set); #1202bv added the first call
    # inside design_prep.py itself, which is scanned, so the shape surfaced only now.
    "design_system_is_enriched",
    "design_prep_reusable_1202bv",
    "run_test_user_squad",  # runtime.test_user_squad python fn, not a tool
    ...
    "run_visual_fidelity",  # internal async fn in runtime/visual_fidelity.py (called by VisualFidelityGate) — not an agent tool
    "run_test_user_validation",  # internal fn in runtime/test_user_validation.py (called by HealPipeline) — not an agent tool
    "run_browser_test_user",  # internal async fn in runtime/test_user_runner.py (called by HealPipeline) — not an agent tool
}
