...
def _reg(step):
    ...
    assert errs == [], errs
    # #40: normalize_steps may AUTO-PREPEND a /auth/register step when a token-auth /api
    # step has no minting step — so return the step matching the input path, not out[0].
    want = str(step.get("path") or step.get("endpoint") or "").split()[-1].rstrip("/")
    for s in out:
        if str(s.get("path") or "").rstrip("/") == want:
            return s
    return out[0]
