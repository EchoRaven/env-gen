...
class ListReferenceImagesContainment(_Fixture):
    ...
    # 9
    def test_list_reference_images_accepts_bundled_library_project(self):
        """``proj_a`` is a bundled-library project and must still be
        listable after the fix.
        """
        tool = self._tool()
        ...
        for entry in (data.get("projects") or {}).values():
            all_names.extend(img.get("name") for img in entry)
        self.assertIn("img.png", all_names)
