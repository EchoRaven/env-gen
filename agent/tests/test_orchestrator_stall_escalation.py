...
class NudgeSilentResidentLanesTests(unittest.TestCase):
    ...
    def test_lane_with_post_finalize_status_not_nudged(self):
        """A lane that emitted agent_status AFTER kickoff_finalized_at AND
        RECENTLY (V30 freshness, not a one-time past anchor) is provably
        alive and MUST NOT be nudged."""
        now = time.time()
        kickoff_finalized_at = now - 1000.0
        statuses = {
            "backend": {"_event_created_at": now - 5.0, "status": "active"},
            "frontend": {"_event_created_at": now - 10.0, "status": "active"},
        }
    ...
    def test_liveness_resets_nudge_counter(self):
        ...
        # Backend wakes and emits a RECENT post-finalize status (V30 freshness).
        stub.hubs.eventhub._statuses["backend"] = {
            "_event_created_at": now - 5.0, "status": "active",
        }
        _run(_nudge_method()(stub, kickoff_finalized_at))
