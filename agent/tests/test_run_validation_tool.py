...
def _build_check_hubs(checks):
    """hubs with a codehub that captures record_check calls."""
    recorded = []

    class _CodeHub:
        def record_check(self, pr_id, name, status, evidence=None, agent="verifier"):
            recorded.append({"pr_id": pr_id, "name": name, "status": status, "agent": agent})
            return {}

    class _Api:
        def get_endpoints(self):
            return {"GET:/api/x": {"method": "GET", "path": "/api/x",
                                   "kind": None, "state": "implemented"}}
    ...
    hubs = types.SimpleNamespace(base_dir=_chains_base(), registryhub=_Api(),
                                 codehub=_CodeHub())
    return hubs, recorded


def _run_with_checks(monkeypatch, checks):
    import tools.validation_tools as vt
    monkeypatch.setattr(vt, "_import_runner",
                        lambda: lambda project_dir, biz, **kw: {
                            "passed": True, "summary": "ok", "checks": checks,
                            "backend_port": 3001, "endpoints": []})
    hubs, recorded = _build_check_hubs(checks)
    t = vt.RunValidationTool(workspace=None)
    t.set_agent(types.SimpleNamespace(agent_id="orchestrator", _hubs=hubs))
    _run(t.execute())
    return {r["name"]: r for r in recorded}


def test_records_build_checks_success_for_delivery_checklist(monkeypatch):
    # run_validation deterministically records the 4 build:* checks the delivery
    # gate's verification checklist keys on, from its own docker-up + health
    # outcome — so a healthy app's checklist goes ready without the verifier
    # manually recording them (smoke run #7: only build:frontend was recorded).
    checks = [{"name": n, "status": "pass"} for n in
              ("docker_up", "backend_health", "frontend_reachable", "business_writes_persist")]
    by_name = _run_with_checks(monkeypatch, checks)
    for comp in ("build:docker", "build:backend", "build:frontend", "build:database"):
        assert by_name[comp]["status"] == "success", comp
        assert by_name[comp]["agent"] == ""        # framework authority
        assert by_name[comp]["pr_id"] == "main"


def test_build_checks_self_correct_on_failure(monkeypatch):
    # a genuinely broken build records failure (keeps the gate honestly blocked),
    # NOT a false success — the frontend reachability failed.
    checks = [{"name": "docker_up", "status": "pass"},
              {"name": "backend_health", "status": "pass"},
              {"name": "frontend_reachable", "status": "fail"},
              {"name": "business_writes_persist", "status": "pass"}]
    by_name = _run_with_checks(monkeypatch, checks)
    assert by_name["build:frontend"]["status"] == "failure"
    assert by_name["build:backend"]["status"] == "success"


def test_build_checks_noop_without_codehub(monkeypatch):
    # hubs lacking a codehub (e.g. older wiring) must not crash the verdict.
    import tools.validation_tools as vt
    monkeypatch.setattr(vt, "_import_runner",
                        lambda: lambda project_dir, biz, **kw: {
                            "passed": True, "summary": "ok", "checks": [], "endpoints": []})
    hubs = types.SimpleNamespace(base_dir=_chains_base(),
                                 registryhub=types.SimpleNamespace(
                                     get_endpoints=lambda: {"GET:/api/x": {"method": "GET", "path": "/api/x", "kind": None, "state": "implemented"}},
                                     record_api_test=lambda *a, **k: {}))
    t = vt.RunValidationTool(workspace=None)
    t.set_agent(types.SimpleNamespace(agent_id="orchestrator", _hubs=hubs))
    res = _run(t.execute())
    assert res.data["passed"] is True
