...
class WriteScopeRoutedByTable(unittest.TestCase):
    ...
    def test_workers_write_code_lanes_but_orchestrator_dispatches(self):
        """Worker-flavoured lane helpers are code-lane writers; the
        orchestrator is deliberately NOT.

        2026-06-24 user directive ("为什么是orchestrator在改代码", encoded in
        path_routed_workspace._lane_writers): coordinators DISPATCH, never
        patch. The orchestrator was caught PATCHING app/backend/ itself
        instead of re-waking the owning lane — a builder/coordinator-split
        violation that also breaks mechanically (it edits its own worktree
        copy, which then conflicts on merge). So ``_lane_writers`` for
        app/backend/ | app/frontend/ | app/database/ is the owning lane +
        its spawned helpers (worker / analysis_worker / review_worker)
        ONLY, excluding the coordinator broad-writers.

        The orchestrator remains a broad writer for SHARED areas (e.g.
        design/), just not for the per-lane application code."""
        # Spawned lane helpers ARE code-lane writers.
        for agent in ("worker", "analysis_worker", "review_worker"):
            self.assertTrue(self.ws.is_write_allowed("app/backend/server.js", agent),
                              f"{agent} (lane helper) should write app/backend/")
            self.assertTrue(self.ws.is_write_allowed("design/spec.api.json", agent),
                              f"{agent} should be allowed under design/")
        # The orchestrator DISPATCHES — it is NOT a code-lane writer.
        self.assertFalse(
            self.ws.is_write_allowed("app/backend/server.js", "orchestrator"),
            "orchestrator must NOT patch lane code (coordinators DISPATCH, "
            "never patch — 2026-06-24 directive); it re-wakes the owning lane.",
        )
        # But the orchestrator IS still a broad writer for shared areas.
        self.assertTrue(
            self.ws.is_write_allowed("design/spec.api.json", "orchestrator"),
            "orchestrator remains a writer for shared design/ artifacts.",
        )
