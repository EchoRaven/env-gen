...
class EndpointNormalizationTests(unittest.TestCase):
    ...
    def test_defaults_auth_required_and_response_key(self):
        ...
        # PROPOSAL #46: business /api/ endpoints get the canonical projector envelope key.
        self.assertEqual(eps[0]["response_key"], "items")  # GET collection
        self.assertEqual(eps[1]["response_key"], "item")   # POST → non-canonical "post" overridden
    ...
    def test_noop_when_all_well_formed(self):
        # PROPOSAL #46: a CANONICAL well-formed key (items/item) is left untouched → noop.
        drafts = {"backend": {"api_endpoints": [
    ...
    def test_control_plane_response_key_preserved(self):
        # PROPOSAL #46 (review must-fix #1): the canonical override is BUSINESS-scoped
        # (/api/ only). A non-/api control-plane endpoint keeps its declared key — the
        # delivery gate is kind-exempt for it and clobbering would cause false drift.
        # #1202ke re-anchor: this used to use /auth/login, which is on the FRAMEWORK'S OWN
        # fixed surface -- and the fixture declared `auth_required: True` for it, which is the
        # very defect #1202ke closes (you cannot hold a token before you log in). The rule
        # under test here is #46's, "a NON-/api endpoint keeps its declared key", so exercise
        # it on a non-/api endpoint the framework does not fix.
        drafts = {"backend": {"api_endpoints": [
            {"method": "POST", "path": "/webhooks/stripe", "response_key": "access_token",
             "auth_required": True}]}}
        nd, notes = _normalize_backend_endpoints_for_reconcile(drafts)
        self.assertEqual(nd["backend"]["api_endpoints"][0]["response_key"], "access_token")
        self.assertEqual(notes, [])

        # ...and a FIXED endpoint keeps its declared key too (it never reaches #46's branch).
        drafts = {"backend": {"api_endpoints": [
            {"method": "POST", "path": "/auth/login", "response_key": "access_token",
             "auth_required": False}]}}
        nd, notes = _normalize_backend_endpoints_for_reconcile(drafts)
        self.assertEqual(nd["backend"]["api_endpoints"][0]["response_key"], "access_token")
        self.assertEqual(notes, [])
