...
class DebuggerPromptTests(unittest.TestCase):
    ...
    def test_system_prompt_requires_root_cause_before_assignment(self) -> None:
        # STALE-TEST drift: the prompt writes the token as ROOT_CAUSE / ROOT-CAUSE
        # (e.g. "ASSIGN WITH EXPLICIT ROOT_CAUSE HYPOTHESIS"), never the space form
        # "ROOT CAUSE". Normalize separators so the intent check (a root-cause hypothesis
        # is required before assignment) matches the actual token form.
        normalized = self.system.upper().replace("_", " ").replace("-", " ")
        self.assertIn("ROOT CAUSE", normalized)
