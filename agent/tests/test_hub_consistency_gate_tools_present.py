...
to call ``registryhub_register_ui_page(name=..., ...)`` to satisfy the
gate. But the ``workhub_tools`` bundle only included
``workhub_create_document`` — not ``registryhub_register_ui_page``. Worse:
``_count_owned_pages`` specifically requires ``kind=="ui_page"``,
and only ``registryhub_register_ui_page`` writes that kind, so the agent
...
class GateDemandedToolsAreInEveryDemandingProfile(unittest.TestCase):
    ...
    History of why this test exists:
      * Earlier round caught ``frontend`` was wired to
        ...
        ``registryhub_register_ui_page``, agent couldn't call it. Fix:
        added ``registryhub_register_ui_page`` to the bundle.
