...
class WorkHubRetroTests(unittest.TestCase):
    ...
    def test_list_retros_filters_only_retro_kind(self) -> None:
        self.reg.workhub.create_document(title="design", agent="d", kind="design")
        self.reg.workhub.create_document(title="r1", agent="orch", kind="retro",
    ...
    def test_get_latest_retro_for_generation_returns_match(self) -> None:
        self.reg.workhub.create_document(title="r-old", agent="orch", kind="retro",
                                       metadata={"generation_id": 1000.0})
        self.reg.workhub.create_document(title="r-new", agent="orch", kind="retro",
    ...
    def test_get_latest_retro_returns_most_recent_when_multiple_for_same_gen(self) -> None:
        ...
        p1 = self.reg.workhub.create_document(title="r1", agent="orch", kind="retro",
                                            metadata={"generation_id": 1000.0})
        time.sleep(0.01)
        p2 = self.reg.workhub.create_document(title="r2", agent="orch", kind="retro",
                                            metadata={"generation_id": 1000.0})
