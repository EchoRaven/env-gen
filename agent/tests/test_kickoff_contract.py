...
class RoundTripIntoRegistryHubTests(unittest.TestCase):
    ...
    def test_normalize_then_register_stores_metadata_correctly(self) -> None:
        ...
        # PROPOSAL #50: register_endpoint canonicalizes a business endpoint's response_key
        # to the projector envelope key — GET /api/posts (collection) -> "items" (was the
        # non-canonical resource name "posts"). The load-bearing check is that it lands in
        # metadata; the value is now canonical.
        self.assertEqual(stored["metadata"]["response_key"], "items")
