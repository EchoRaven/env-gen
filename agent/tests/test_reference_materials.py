...
class MilestoneSliceCarriesSpecTests(unittest.TestCase):
    def test_orchestrator_appends_spec_summary_at_m1_only(self):
        """The compiled reference-spec block is a LAST-RESORT backstop on a milestone's
        requirement — re-appended ONLY when that phase has neither an authored detail NOR a
        rough slice (found live 2026-06-10: M1 kickoff lost the binding endpoint/MCP lists).
        Per-milestone planning (P1/P2/P4, 2026-06-24) normally scopes each phase by its own
        detail/slice, so the spec block must NOT be appended to phases that already carry one
        (else M1 over-declares the whole surface and later milestones get 0 new — smoke-notes
        2026-06-19: M2 empty-screens rejection)."""
        ...
        # the milestone-requirement is built from the slice/detail …
        self.assertIn('_milestone.get("description_slice")', src)
        # … and the spec block is a backstop appended ONLY when there's neither detail nor
        # slice (both unique strings — their presence confirms the redesigned backstop).
        self.assertIn("not (_detail and _detail.strip()) and not _slice", src)
        guard = src.index("not (_detail and _detail.strip()) and not _slice")
        self.assertIn("_milestone_req = _milestone_req + _spec_block", src[guard:guard + 300])
...
class DecisionFileAuthoringTests(unittest.TestCase):
    ...
    def test_decision_file_from_worktree(self):
        ...
        page = reg.workhub.get_document(meeting_id)
...
class DeclareToolsTests(unittest.TestCase):
    ...
    def test_pages_flows_predicates_endpoints_tables_merge(self):
        ...
        page = reg.workhub.get_document(mid)
...
class MilestonePlanningTests(unittest.TestCase):
    ...
    def test_planner_free_mode_honors_k(self):
        ...
        # FREE mode (no ENVGEN_MILESTONES): the planner's chosen K is honored — NO artificial
        # ceiling. The old data[:6] truncation silently dropped a complex app's later phases.
        out = asyncio.run(plan_milestones(
            self._fake_llm(json.dumps(plan)), "req", {"endpoints": []}))
        self.assertEqual(len(out), 8)
        self.assertEqual(out[0]["version"], "1.0.0")

    def test_planner_force_mode_caps_to_n(self):
        import os
        from multi_agent.runtime.reference_materials import plan_milestones
        plan = [{"name": f"M{i+1}-x", "version": f"1.{i}.0",
                 "description_slice": f"slice {i}"} for i in range(8)]
        os.environ["ENVGEN_MILESTONES"] = "5"  # FORCE exactly N → truncates
        try:
            out = asyncio.run(plan_milestones(
                self._fake_llm(json.dumps(plan)), "req", {"endpoints": []}))
        finally:
            os.environ.pop("ENVGEN_MILESTONES", None)
    ...
    def test_orchestrator_honors_explicit_and_plans_otherwise(self):
        src = Path(THIS_DIR.parent / "env_generator" / "llm_generator" /
                   "multi_agent" / "orchestrator.py").read_text()
        # planning fires when the user gave no explicit milestones (now also guarded by the
        # force-single-milestone escape hatch — assert the stable prefix).
        self.assertIn("if not _milestones_explicit and", src)
...
class SkeletonMarksTablesImplementedTests(unittest.TestCase):
    ...
    def test_orchestrator_flips_generated_tables(self):
        # The backend-skeleton table-status flip moved to
        # runtime.scaffolder.Scaffolder.generate_backend_skeleton
        # (PROPOSAL #8 — Scaffolder extraction); the orchestrator delegates.
...
class UiComponentModelTests(unittest.TestCase):
    ...
    def test_page_rollup_blocks_on_unimplemented_component(self):
        ...
        # #1089: implementing a component now includes MOUNTING it — which is exactly what
        # this page already claimed by declaring `components: ["filter_rail"]`. The fixture
        # used to create the file and leave the page rendering null, so it asserted a green
        # rollup for a component the app never showed.
        (src / "pages" / "CatalogPage.jsx").write_text(
            "import { apiGet } from '../services/api';\n"
            "import FilterRail from '../components/FilterRail';\n"
            "export default function CatalogPage() { apiGet('/api/products'); "
            "return <FilterRail />; }\n")
...
class CustomRoutesHookTests(unittest.TestCase):
    ...
    def test_skeleton_main_includes_custom_router(self):
        ...
        # FIX #127 evolved the hook: import the MODULE and include EVERY
        # APIRouter it defines (a second `hidden_router` used to be orphaned).
        # This test's intent is unchanged — the hook must be present + guarded.
        self.assertIn("import custom_routes as _custom_mod", body)
        self.assertIn("app.include_router(_custom_router)", body)
        self.assertIn("except ImportError as _custom_imp:", body)
...
class ChainAuthoringFeedbackTests(unittest.TestCase):
    ...
    def test_orchestrator_dispatches_chain_task(self):
        # business_chain dispatch moved to runtime.framework_validation.FrameworkValidation
        # (PROPOSAL #8/#15 — FrameworkValidation extraction); orchestrator delegates.
        src = Path(__file__).resolve().parent.parent / (
...
class CanonicalLayoutAuditTests(unittest.TestCase):
    ...
    def test_canonical_locations_pass(self):
        ...
        # #1089: this case is about canonical LOCATIONS, and its fixture happened to leave
        # NavBar mounted by nobody — which the component audit now (correctly) reports as
        # dead UI. Mount it, so the case keeps asserting the one thing its name claims.
        (src / "pages" / "HomePage.jsx").write_text(
            "import NavBar from '../components/NavBar';\n"
            "export default function HomePage() { return <NavBar />; }")
...
class UiPageCaseNormalizeTests(unittest.TestCase):
    """Round 38: frontend's legacy registryhub_register_ui_page with the PascalCase
...
class VerifierChainToolSurfaceTests(unittest.TestCase):
    ...
    def test_orchestrator_detects_chains_blocked_exit(self):
        # CHAINS-BLOCKED early-exit moved to runtime.framework_validation.FrameworkValidation
        # (PROPOSAL #8/#15 — FrameworkValidation extraction).
        src = (Path(__file__).resolve().parent.parent /
               "env_generator/llm_generator/multi_agent/runtime/framework_validation.py").read_text()
...
class ChainBodyStringToleranceTests(unittest.TestCase):
    ...
    def test_dict_body_unchanged(self):
        ...
        # #40: normalize_steps auto-prepends a /auth/register step for a token-auth /api
        # step with no minting step, so assert on the /api/x step (not steps[0]).
        api_step = next(s for s in steps if s.get("path") == "/api/x")
        self.assertEqual(api_step["body"], {"a": 1})
...
class ChainAuthFirstReorderTests(unittest.TestCase):
    ...
    def test_non_auth_relative_order_preserved(self):
        # A login with NO register now gets a register synthesized before it
        # (ENSURE-USER-BEFORE-LOGIN, 2026-06-20) so the login has a user to
        # authenticate. The non-auth steps must still keep their relative order
        # (/api/a before /api/b) AFTER the auth round-trip.
        from multi_agent.runtime.chain_executor import normalize_steps
        steps, _ = normalize_steps([
        ...
        self.assertEqual([s["path"] for s in steps],
                         ["/auth/register", "/auth/login", "/api/a", "/api/b"])
...
class FrontendImportedDepAutoAddTests(unittest.TestCase):
    """A lane importing a common lib not in the framework-owned package.json
    (it can't edit it) used to fail the vite/Rollup build → docker_up FAILED
    forever (live smoke-notes 2026-06-20: `import "date-fns"` in NoteCard.jsx).
    pin_frontend_build_tooling now scans src and auto-adds imported common libs."""

    def _reconcile(self, src_files, pkg=None):
        import tempfile, json, os
        from multi_agent.runtime.frontend_scaffold import pin_frontend_build_tooling
        d = tempfile.mkdtemp()
        os.makedirs(d + "/src/components", exist_ok=True)
        open(d + "/package.json", "w").write(json.dumps(
            pkg or {"dependencies": {"react": "^18.3.1", "react-dom": "^18.3.1"}}))
        for rel, txt in src_files.items():
            path = d + "/src/" + rel
            os.makedirs(os.path.dirname(path), exist_ok=True)
            open(path, "w").write(txt)
        pin_frontend_build_tooling(d)
        return json.load(open(d + "/package.json")).get("dependencies", {})

    def test_imported_date_fns_is_added(self):
        deps = self._reconcile({
            "components/NoteCard.jsx": "import { format } from 'date-fns';\nexport default 1;",
        })
        self.assertIn("date-fns", deps)
        self.assertEqual(deps["date-fns"], "^3.6.0")

    def test_subpath_and_scoped_imports_resolve_to_root(self):
        deps = self._reconcile({
            "a.jsx": "import debounce from 'lodash/debounce';\n",
            "b.jsx": "import { useQuery } from '@tanstack/react-query';\n",
        })
        self.assertIn("lodash", deps)
        self.assertIn("@tanstack/react-query", deps)

    def test_relative_and_builtin_imports_not_added(self):
        deps = self._reconcile({
            "a.jsx": "import Foo from './Foo';\nimport React from 'react';\nimport api from '../services/api.js';",
        })
        # react already present (not duplicated), relative imports ignored, and
        # no stray deps invented.
        self.assertEqual(set(deps), {"react", "react-dom", "react-router-dom"})

    def test_uncurated_package_added_as_latest(self):
        # GENERALITY (youtube 2026-06-20): a curated allowlist can never cover
        # every real lib an app uses (lucide-react was missing → Rollup "failed to
        # resolve" → docker_up FAILED → no delivery). Every installable bare import
        # is now added — pinned if known, "latest" otherwise — so a real uncurated
        # lib builds. A hallucinated NAME fails at `npm install` (equally
        # triageable), which is strictly better than failing EVERY uncurated-lib app.
        deps = self._reconcile({
            "a.jsx": "import x from 'some-real-but-uncurated-lib';\n",
        })
        self.assertEqual(deps.get("some-real-but-uncurated-lib"), "latest")

    def test_virtual_and_protocol_specifiers_not_added(self):
        # the installable-name guard keeps node:/virtual: out of package.json.
        deps = self._reconcile({
            "a.jsx": "import 'virtual:uno.css';\nimport fs from 'node:fs';\n",
        })
        self.assertNotIn("virtual:uno.css", deps)
        self.assertNotIn("node:fs", deps)

    def test_already_present_dep_not_overwritten(self):
        deps = self._reconcile(
            {"a.jsx": "import axios from 'axios';\n"},
            pkg={"dependencies": {"react": "^18.3.1", "axios": "^1.0.0"}})
        self.assertEqual(deps["axios"], "^1.0.0")  # lane/existing pin preserved
