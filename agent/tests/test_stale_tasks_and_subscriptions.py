...
class DefaultSubscriptionsWakeAgents(unittest.TestCase):
    ...
    def test_ensure_default_subscriptions_installs_them_idempotently(self) -> None:
        ...
        # PROPOSAL #26: backend now also carries the inbox_only framework_decision
        # subscription, so the total is DEFAULT + INBOX_ONLY for the lane.
        from multi_agent.runtime.agent_subscriptions import INBOX_ONLY_SUBSCRIPTIONS
        expected_count = (len(DEFAULT_SUBSCRIPTIONS["backend"])
                          + len(INBOX_ONLY_SUBSCRIPTIONS.get("backend", [])))
        self.assertEqual(len(subs), expected_count)

    def test_endpoint_defined_is_pulse_only_not_inbox_clutter(self) -> None:
        """endpoint_defined is a PULSE-ONLY contract event (run v12: these were
        ~75% of a lane's inbox). A fresh registration is recorded in the events
        store — so a lane reads it from the registry pulse — but it deliberately
        does NOT land in a subscriber's inbox (no per-registration wakeup churn).
        endpoint_schema_changed stays live so a contract DRIFT still reaches
        consumers; this pins the no-clutter side."""
        ensure_default_subscriptions(self.hubs, "backend")
        ...
        self.assertNotIn("endpoint_defined", types,
                         f"endpoint_defined is pulse-only; must NOT clutter the inbox: {types}")
