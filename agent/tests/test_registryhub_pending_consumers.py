...
class TestPendingConsumerFlow(unittest.TestCase):
    ...
    def test_pending_promoted_on_endpoint_registration(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # And an urgent endpoint_implemented event was emitted to backend.
            # endpoint_implemented is a PULSE-ONLY contract-lifecycle event
            # (eventhub._PULSE_ONLY_EVENT_TYPES): it is RECORDED in the events
            # store (dashboard timeline + queryable, and consumers see the
            # promotion via the registry pulse) but creates NO inbox item, so
            # assert on the events store — not list_inbox.
            store = reg.eventhub._events.value()
            implemented_events = [
                e for e in store.values()
                if isinstance(e, dict)
                and e.get("event_type") == "endpoint_implemented"
                and e.get("payload", {}).get("path") == "/api/users"
                and "backend" in (e.get("recipients") or [])
            ]
            self.assertEqual(len(implemented_events), 1,
                             f"no endpoint_implemented event addressed to backend; "
                             f"got: {[e.get('event_type') for e in store.values() if isinstance(e, dict)]}")
