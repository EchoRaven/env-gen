...
def _vf_orch_stub():
    """Orchestrator stand-in (the gate's I/O collaborator) — it no longer carries
    the _vf_* state; VisualFidelityGate owns that now (PROPOSAL #8 — slice B).
    Its app-source signature is "NEW_SIG" to simulate frontend-lane churn."""
...
def test_visual_fidelity_does_not_reset_deferral_clock_on_sig_change(monkeypatch):
    ...
    gate = VisualFidelityGate(_vf_orch_stub())
    gate.sig = "OLD_SIG"
    gate.attempts = 2
    gate.passed = False
    gate.deferred_since = 5_000.0   # a prior defer already anchored this milestone
    gate.total_judgments = 1
    gate.last_judged_sig = "SOME_OTHER_SIG"
    _run(gate.maybe_run())

    # deferral clock PRESERVED across the sig change (the bug was resetting it to None)
    assert gate.deferred_since == 5_000.0
    # per-source attempt budget WAS reset by the sig change, then this judgment counted
    assert gate.attempts == 1
    assert gate.sig == "NEW_SIG"
    # per-milestone total-judgment backstop advanced (NOT reset on sig change)
    assert gate.total_judgments == 2
...
def test_visual_fidelity_pass_latches(monkeypatch):
    ...
    gate = VisualFidelityGate(_vf_orch_stub())
    gate.last_judged_sig = "SOME_OTHER_SIG"  # force a real judge
    _run(gate.maybe_run())
...
def _fwval_stub(*, run_data, attempts=0, healed_sig="SIG_A", impl_count=2):
    ...
    stub = types.SimpleNamespace(
        _logger=MagicMock(),
        progress=MagicMock(),
        # #482's per-tick app/database scaffold reads orch.output_dir. Without it
        # the whole validation raised and was swallowed as "non-fatal", so the tick
        # did nothing at all — neither the stuck ladder nor the attempt counter
        # moved, and every assertion below read a value that was never computed.
    ...
    for _m in ("_merge_committed_agent_work", "_scaffold_design_readme",
               ...
               "_repair_frontend_api", "_scaffold_frontend_pages",
               "_repair_backend_entrypoint",
               ...
               "_commit_framework_delivery"):  # #41 v2: per-tick baseline commit
...
def test_fix112_escape_window_allows_multiple_remediation_rounds():
    """FIX #112 (runs 24+26 autopsy): remediation rounds take 3-10 min each and
    scores DO rise +0.1-0.4/round, but the 900s anchored escape fired after only
    1-3 rounds — the gate released below threshold while the lane was still
    converging. With time-unlimited runs (user-approved) the window must fit
    5-6 rounds; #112b (user 2026-07-08: "900s还是太短...可以长一点"): a full hour
    (~6-15 rounds), judgment cap raised so it can't become the binding constraint."""
    assert VISUAL_DEFERRAL_ESCAPE_S >= 3600
    assert VISUAL_TOTAL_JUDGMENTS_CAP >= 20
    # 45 minutes into the deferral with rounds left: must still defer
    assert _visual_release_decision(deferred_since=1_000, attempts=1,
                                    total_judgments=8, now=1_000 + 2_700) == "defer"
