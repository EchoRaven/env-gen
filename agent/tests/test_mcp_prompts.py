...
class BackendMCPPromptTests(unittest.TestCase):
    ...
    def test_describes_runtime_projected_mcp_server(self) -> None:
        # The MCP server is now RUNTIME-projected from the registered endpoints
        # (one ``@mcp.tool`` per endpoint); the backend agent no longer authors
        # the server or chooses its transport (STDIO/HTTP is a runtime concern,
        # handled in runhub/scaffolder). The prompt must teach that projection
        # discipline rather than transport options.
        upper = self.system.upper()
        self.assertIn("PROJECTED", upper)
        self.assertIn("@MCP.TOOL", upper)
