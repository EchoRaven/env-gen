...
``live_monitor_server.py`` we ask, via Python's ``ast`` module:
  ...
  Q1. Does this function mutate state? Heuristics:
        ...
        * Hub mutation method calls — any ``.codehub.X``,
          ...
          ``.create_document`` / ``.append_block`` / ``.update_block`` /
          ``.insert_block_after`` /
...
KNOWN_DEFERRED_TO_EXT: frozenset = frozenset({
    ...
    # Page→document persistence migration: the handler formerly named
    # ``workhub_create_page_call`` is now ``workhub_create_document_call``
    # (live_monitor_server.py:3737 — POST .../workhub/pages still, but the
    # handler + hub method were renamed create_page → create_document). Same
    # ungated posture as before (no role check; body.agent forwarded), so the
    # deferral simply follows the rename — this is NOT a closed bypasser.
    "workhub_create_document_call",
