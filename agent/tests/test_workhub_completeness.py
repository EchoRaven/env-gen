...
class TestGetPage(unittest.TestCase):
    def test_get_page_returns_page_and_blocks(self):
        ...
        hub = _hub()
        page = hub.create_page("Spec", agent="orch")
        hub.append_block(page["id"], {"type": "text", "content": "hello"}, agent="orch")
        result = hub.get_page(page["id"], with_blocks=True)
    ...
    def test_get_page_without_blocks(self):
        ...
        page = hub.create_page("Spec", agent="orch")
        hub.append_block(page["id"], {"type": "text", "content": "hello"}, agent="orch")
        result = hub.get_page(page["id"], with_blocks=False)
        self.assertEqual(result["id"], page["id"])
    ...
    def test_get_page_missing_returns_none(self):
        ...
        hub = _hub()
        self.assertIsNone(hub.get_page("page_nope"))
...
class TestListPages(unittest.TestCase):
    def test_list_pages_returns_all_pages(self):
        ...
        hub = _hub()
        hub.create_page("Page A", agent="orch")
        hub.create_page("Page B", agent="orch")
        pages = hub.list_pages()
        self.assertGreaterEqual(len(pages), 2)

    def test_list_pages_filter_by_status(self):
        """list_pages(status='active') returns only active pages."""
        hub = _hub()
        hub.create_page("Active Page", agent="orch")
        pages = hub.list_pages(status="active")
    ...
    def test_list_pages_filter_by_kind(self):
        ...
        hub = _hub()
        p1 = hub.create_page("Spec Page", agent="orch")
        # Manually set kind on the page
        ts = "orch"
        updated = dict(p1)
        updated["kind"] = "spec"
        hub.stores.pages.update(lambda m: m.set(p1["id"], updated, ts))
        hub.create_page("Notes Page", agent="orch")
        result = hub.list_pages(kind="spec")
...
class TestArchivePage(unittest.TestCase):
    def test_archive_page_sets_status_archived(self):
        ...
        hub = _hub()
        page = hub.create_page("Old Spec", agent="orch")
        result = hub.archive_page(page["id"], agent="orch")
    ...
    def test_archive_page_unknown_page_returns_error(self):
        ...
        hub = _hub()
        result = hub.archive_page("page_nope", agent="orch")
        self.assertIn("error", result)
    ...
    def test_archive_page_persists(self):
        ...
        hub = _hub()
        page = hub.create_page("Old Spec", agent="orch")
        hub.archive_page(page["id"], agent="orch")
        stored = hub.stores.pages.value()[page["id"]]
...
class TestRemoveAttendee(unittest.TestCase):
    def test_remove_attendee_soft_deletes(self):
        ...
        hub = _hub()
        page = hub.create_page("Spec", attendees=["frontend"], agent="orch")
    ...
    def test_remove_attendee_uses_composite_key(self):
        ...
        hub = _hub()
        page = hub.create_page("Spec", attendees=["backend"], agent="orch")
