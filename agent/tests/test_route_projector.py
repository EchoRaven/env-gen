...
def test_search_handler_columns_are_domain_general(tmp_path):
    """GENERALITY: the projected search handler searches the model's ACTUAL textual
    columns (derived from the contract type map) — not a hardcoded social/content
    vocabulary. So it searches whatever text columns an app declares, and never a
    column the model lacks. Secrets and non-text columns are excluded."""
    be = _backend(tmp_path)
    ...
    # the User model's real text columns are searched (username/full_name=String, bio=Text)
    for textcol in ("username", "full_name", "bio"):
        assert f"'{textcol}'" in line or f'"{textcol}"' in line, f"text column {textcol!r} not searched"
    # secrets + non-text columns are NOT searched
    assert "password_hash" not in line          # secret, even though it is a String column
    assert "'id'" not in line and "'created_at'" not in line   # Integer / DateTime
    # NOT a hardcoded vocabulary the model doesn't have
    assert "caption" not in line and "description" not in line


def test_search_handler_runtime_type_guards_ilike(tmp_path):
    """A projected search must RUNTIME-guard ``ilike`` to String/Text columns — when the
    contract type map is unavailable the cols fall back to ALL columns, and calling
    ``.ilike`` on an Integer/Boolean/DateTime column raises (outlook GET
    /api/messages/search → 500). The guard skips non-text columns at request time."""
    import ast as _ast
    be = _backend(tmp_path)
    project_missing_routes(be, [{"method": "GET", "path": "/api/users/search"}])
    src = (be / "main.py").read_text(encoding="utf-8")
    _ast.parse(src)
    handler = src[src.find('@app.get("/api/users/search")'):]
    handler = handler.split("\n\n\n")[0]
    assert "String as _Str, Text as _Txt" in handler
    assert "isinstance(getattr(getattr(" in handler and "(_Str, _Txt))" in handler, handler
...
def test_create_injects_authenticated_owner(tmp_path):
    ...
    src = (be / "main.py").read_text(encoding="utf-8")
    ast.parse(src)
    assert 'valid.setdefault("author_id", _fw_owner_val(Post, "author_id", user))' in src
...
def test_nested_create_binds_parent_target_and_owner(tmp_path):
    ...
    assert 'valid.setdefault("follower_id", _fw_owner_val(Follow, "follower_id", user))' in src   # actor = authenticated caller
