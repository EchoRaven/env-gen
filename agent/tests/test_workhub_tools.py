...
REQUIRED_WORKHUB_TOOLS = {
    ...
    # renamed from ``workhub_archive_page`` in commit 84f658e — the WorkHub
    # coordination-doc concept was renamed 'page'→'document' (full separation
    # from RegistryHub UI pages).
...
class WorkHubToolsExportTests(unittest.TestCase):
    ...
    def test_workhub_comments_for_tool_returns_comments(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            hubs, tool_by_name = self._hubs_and_tools(td)
            page = hubs.workhub.create_document("MyPage", agent="backend")
