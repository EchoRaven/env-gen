"""Minimum 11-test OAuth contract for the Telegram sandbox env.

Spec: `.claude/skills/env-oauth-blueprint/SKILL.md`. Run against a live
telegram-api stack:

    docker compose up -d   (from src/envs/telegram)
    TELEGRAM_API_URL=http://localhost:8038 python3 tests/oauth/test_telegram.py
"""
...
API = os.getenv("TELEGRAM_API_URL", "http://localhost:8038")
EXPECTED_AUDIENCE = os.getenv("TELEGRAM_EXPECTED_AUDIENCE", "telegram-api")
...
def test_health_and_discovery() -> None:
    ...
    body = d.json()
    _assert("telegram.read" in body["scopes_supported"], "scopes_supported missing telegram.read")
    _assert("telegram.write" in body["scopes_supported"], "scopes_supported missing telegram.write")
...
def test_register_and_login_returns_jwt() -> None:
    ...
    _assert("telegram.read" in claims["scope"].split(), "scope includes telegram.read")
...
def test_jwt_works_against_protected_route() -> None:
    """A JWT should authenticate REST routes via Authorization: Bearer."""
    tid = _unique("t")
    email = _unique("u") + "@test.com"
    _setup_tenant_user(tid, email)
    ...
    _assert(r.status_code == 200, f"/contacts {r.status_code}: {r.text[:200]}")
    body = r.json()
    _assert("contacts" in body, "contacts key in response")
    _record("UI JWT → /contacts", True)


def test_garbage_token_rejected() -> None:
    r = requests.get(f"{API}/contacts", headers={"Authorization": "Bearer notajwt"})
...
def test_tampered_token_rejected() -> None:
    ...
    r = requests.get(f"{API}/contacts", headers={"Authorization": f"Bearer {tampered}"})
...
def test_oauth_dance_end_to_end() -> None:
    ...
    form = {
        ...
        "scope": "telegram.read telegram.write",
        "tenant_id": tid,
        "email": email,
        "password": "pw",
    }
...
TESTS = [
    ...
    test_jwt_works_against_protected_route,
