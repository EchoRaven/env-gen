"""Minimum 10-test OAuth contract for the WhatsApp sandbox env.

Spec: `.claude/skills/env-oauth-blueprint/SKILL.md`. Run against a live
whatsapp-api stack:

    docker compose up -d   (from src/envs/whatsapp)
    WHATSAPP_API_URL=http://localhost:8039 python3 tests/oauth/test_whatsapp.py
"""
...
API = os.getenv("WHATSAPP_API_URL", "http://localhost:8039")
EXPECTED_AUDIENCE = os.getenv("WHATSAPP_EXPECTED_AUDIENCE", "whatsapp-api")
...
def _setup_tenant_user(tenant_id: str, email: str, password: str = "pw") -> dict:
    ...
    r = requests.post(
        f"{API}/auth/register",
        json={"email": email, "name": email.split("@")[0].title(), "password": password, "phone": "+1-555-0000"},
...
def test_health_and_discovery() -> None:
    ...
    body = d.json()
    _assert("whatsapp.read" in body["scopes_supported"], "scopes_supported missing whatsapp.read")
    _assert("whatsapp.write" in body["scopes_supported"], "scopes_supported missing whatsapp.write")
...
def test_register_and_login_returns_jwt() -> None:
    ...
    _assert("whatsapp.read" in claims["scope"].split(), "scope includes whatsapp.read")
...
def test_jwt_works_against_tools_call() -> None:
    """A JWT should authenticate /tools/call (passed via arguments.access_token)."""
    tid = _unique("t")
    email = _unique("u") + "@test.com"
    _setup_tenant_user(tid, email)
    ...
    r = requests.post(
        f"{API}/tools/call",
        json={"name": "list_contacts", "arguments": {"access_token": token}},
    )
    _assert(r.status_code == 200, f"/tools/call {r.status_code}: {r.text[:200]}")
    body = r.json()
    _assert("result" in body, "result key in response")
    _record("UI JWT → /tools/call list_contacts", True)


def test_garbage_token_rejected() -> None:
    r = requests.post(
        f"{API}/tools/call",
        json={"name": "list_contacts", "arguments": {"access_token": "notajwt"}},
    )
...
def test_no_token_rejected() -> None:
    r = requests.post(
        f"{API}/tools/call",
        json={"name": "list_contacts", "arguments": {}},
    )
...
def test_tampered_token_rejected() -> None:
    ...
    r = requests.post(
        f"{API}/tools/call",
        json={"name": "list_contacts", "arguments": {"access_token": tampered}},
    )
...
TESTS = [
    ...
    test_jwt_works_against_tools_call,
