"""Minimum 11-test OAuth contract for the Zoom sandbox env.

Spec: `.claude/skills/env-oauth-blueprint/SKILL.md`. Run against a live
zoom-api stack:

    docker compose up -d   (from src/envs/zoom)
    ZOOM_API_URL=http://localhost:8033 python3 tests/oauth/test_zoom.py
"""
...
API = os.getenv("ZOOM_API_URL", "http://localhost:8033")
EXPECTED_AUDIENCE = os.getenv("ZOOM_EXPECTED_AUDIENCE", "zoom-api")
...
def _setup_tenant_user(tenant_id: str, email: str, password: str = "pw") -> dict:
    ...
    r = requests.post(
        f"{API}/auth/register",
        json={"email": email, "name": email.split("@")[0].title(), "password": password, "tenant_id": tenant_id},
    )
    r.raise_for_status()
    return r.json()
...
def test_health_and_discovery() -> None:
    ...
    body = d.json()
    _assert("zoom.read" in body["scopes_supported"], "scopes_supported missing zoom.read")
    _assert("zoom.write" in body["scopes_supported"], "scopes_supported missing zoom.write")
...
def test_register_and_login_returns_jwt() -> None:
    ...
    _assert("zoom.read" in claims["scope"].split(), "scope includes zoom.read")
...
def test_duplicate_register_rejected() -> None:
    ...
    r = requests.post(
        f"{API}/auth/register",
        json={"email": email, "name": "Dup", "password": "pw", "tenant_id": tid},
    )
...
def test_jwt_works_against_protected_route() -> None:
    ...
    _assert(r.status_code == 200, f"/api/v1/me {r.status_code}: {r.text[:200]}")
    body = r.json()
    _assert(body["email"] == email, "me.email")
    _assert(body["tenant_id"] == tid, "me.tenant_id")
    _record("UI JWT → /api/v1/me", True)


def test_garbage_token_rejected() -> None:
    r = requests.get(f"{API}/api/v1/me", headers={"Authorization": "Bearer notajwt"})
...
def test_tampered_token_rejected() -> None:
    ...
    r = requests.get(f"{API}/api/v1/me", headers={"Authorization": f"Bearer {tampered}"})
