"""Minimum 8-test OAuth contract for the PayPal sandbox env.

Spec: `.claude/skills/env-oauth-blueprint/SKILL.md`. Run against a live
paypal-api stack:

    docker compose up -d   (from src/envs/paypal)
    PAYPAL_API_URL=http://localhost:8035 python3 tests/oauth/test_paypal.py
"""
...
API = os.getenv("PAYPAL_API_URL", "http://localhost:8035")
EXPECTED_AUDIENCE = os.getenv("PAYPAL_EXPECTED_AUDIENCE", "paypal-api")
...
def test_health_and_discovery() -> None:
    ...
    body = d.json()
    _assert("paypal.read" in body["scopes_supported"], "scopes_supported missing paypal.read")
    _assert("paypal.write" in body["scopes_supported"], "scopes_supported missing paypal.write")
...
def test_register_and_login_returns_jwt() -> None:
    ...
    _assert("paypal.read" in claims["scope"].split(), "scope includes paypal.read")
...
def test_jwt_works_against_auth_me() -> None:
    ...
    _assert(r.status_code == 200, f"/auth/me {r.status_code} {r.text[:200]}")
    body = r.json()
    _assert(body["email"] == email, "me.email")
    _assert(body["tenant_id"] == tid, "me.tenant_id")
    _assert("access_token" not in body, "/auth/me must not echo any token (legacy column dropped)")
    _record("UI JWT → /auth/me", True)


def test_garbage_token_rejected() -> None:
    r = requests.get(f"{API}/auth/me", headers={"Authorization": "Bearer notajwt"})
...
def test_tampered_token_rejected() -> None:
    ...
    r = requests.get(f"{API}/auth/me", headers={"Authorization": f"Bearer {tampered}"})
...
def test_oauth_dance_end_to_end() -> None:
    ...
    form = {
        ...
        "scope": "paypal.read paypal.write",
        "tenant_id": tid,
        "email": email,
        "password": "pw",
    }
