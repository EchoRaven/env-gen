...
_CURRENT_MILESTONE_INSTRUCTIONS = """You are the ORCHESTRATOR entering a new milestone of a MULTI-milestone build. Do TWO things:

1) DETAILED BRIEF for THIS milestone (REQUIRED): expand this milestone's rough scope into a concrete, actionable brief the build lanes (backend / frontend / verifier) will execute at kickoff. Be specific and bounded:
   - exactly which endpoints, pages/screens, components, and data this phase ADDS;
   - the acceptance for THIS phase (how we know it's done);
   - what is ALREADY shipped by earlier milestones and must NOT be rebuilt or re-scoped.
   Scope it to THIS phase ONLY — do NOT restate the whole app as this milestone's work.

2) ROADMAP REVISION for the REMAINING (not-yet-started) milestones (OPTIONAL): review the milestones AFTER this one against what's actually been delivered so far. If the plan should change (split / merge / reorder / re-scope future phases), output the revised remaining list. If no change is warranted, OMIT the field — a no-op is a perfectly valid outcome. You decide.

HARD RULES: already-delivered milestones are FROZEN — never re-scope or rebuild them. Across all milestones the build must STILL cover every endpoint/screen of the overall goal by the end.

Output STRICT JSON, nothing else:
{
  "brief": "<detailed, concrete brief for THIS milestone — markdown ok>",
  "revised_remaining": [ {"name": "<short>", "version": "<x.y.z>", "description_slice": "<scope of that future phase>", "acceptance": ["..."]}, ... ]
}
The "revised_remaining" key is OPTIONAL — include it ONLY if you are changing the future plan; it lists the milestones AFTER this one, in order."""


async def plan_current_milestone(
    llm: Any,
    raw_requirements: str,
    roadmap: List[Mapping[str, Any]],
    milestone_index: int,
    current_slice: str,
    delivered_summary: str = "",
) -> Optional[Dict[str, Any]]:
    """At the START of each milestone the orchestrator authors a DETAILED brief for
    THIS milestone (P1) and MAY revise the REMAINING roadmap (P2). The step ALWAYS
    runs (the flow is mandatory); a no-op revision is a valid outcome the orchestrator
    chooses. Returns ``{"brief": str, "revised_remaining": Optional[list]}`` or None on
    any failure (caller falls back to the static slice). One call with the run's model."""
    from utils.llm import Message
    roadmap_text = "\n".join(
        f"  M{i + 1} [{m.get('name')}@{m.get('version')}]: "
        f"{str(m.get('description_slice', ''))[:300]}"
        for i, m in enumerate(roadmap or []))
    prompt = (
        _CURRENT_MILESTONE_INSTRUCTIONS
        + "\n\n## OVERALL PROJECT GOAL (the full end target)\n"
        + str(raw_requirements or "")[:14_000]
        + "\n\n## THE ROADMAP (all milestones)\n" + roadmap_text
        + "\n\n## ALREADY DELIVERED (frozen — do NOT re-scope or rebuild)\n"
        + (delivered_summary or "(nothing yet — this is the first milestone)")
        + f"\n\n## YOU ARE ENTERING: milestone {milestone_index} of {len(roadmap or [])}\n"
        + "Its current rough scope:\n" + str(current_slice or "")[:2_000])
    try:
        client = getattr(llm, "_client", llm)
        resp = await client.chat([Message.user_multimodal(
            [{"type": "text", "text": prompt}])], temperature=0.0, max_tokens=8000)
        text = getattr(resp, "content", "") or ""
        m = re.search(r"\{.*\}", text, re.DOTALL)
        if not m:
            return None
        ...
        if not brief:
            return None
        rev_out: Optional[List[Dict[str, str]]] = None
        revised = data.get("revised_remaining")
        if isinstance(revised, list) and revised:
            rev_out = []
            for i, e in enumerate(revised):
                if not isinstance(e, Mapping):
                    rev_out = None
                    break
                slc = str(e.get("description_slice") or "").strip()
                if not slc:
                    rev_out = None
                    break
                _acc = e.get("acceptance")
                rev_out.append({
                    "name": str(e.get("name") or f"M{milestone_index + i}").strip(),
                    "version": str(e.get("version") or f"1.{milestone_index + i}.0").strip(),
                    "description_slice": slc,
                    "acceptance": [str(a) for a in _acc] if isinstance(_acc, list) else [],
                })
        return {"brief": brief, "revised_remaining": rev_out}
    except Exception:
        return None
