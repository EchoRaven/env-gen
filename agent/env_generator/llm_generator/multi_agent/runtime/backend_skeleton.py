...
def render_seed_data(tables: Dict[str, Any], bootstrap_spec: Optional[List[Dict[str, Any]]] = None) -> str:
    ...
    body = (
        ...
        "                    continue\n"
        "                # #1202sc: compare ids as TEXT. A TEXT primary key holding '1' and a\n"
        "                # dependent row whose FK carries the integer 1 are the SAME row --\n"
        "                # postgres coerces on insert, and #808 aligns the dataset's ids to the\n"
        "                # declared pk type while nothing aligns the LANE's fk values. Comparing\n"
        "                # them raw made the id spaces look foreign when only the JSON types\n"
        "                # differed: r103, r125 and r55 lost their whole dataset that way.\n"
        "                _new_ids = {str(_r.get('id')) for _r in _rows\n"
        "                            if isinstance(_r, dict) and _r.get('id') is not None}\n"
        ...
        "                                continue\n"
        "                            _dep_total += 1\n"
        "                            if str(_d.get(_fk)) not in _new_ids:\n"
        "                                _dep_orphan += 1\n"
        ...
        "                             next(iter(sorted(_new_ids)), None)))\n"
        "            for _t, _rows in real.items():\n"
        "                if _t in _refused:\n"
        "                    continue\n"
        ...
        "                # #1202rp: CARRY THE COLUMNS THE DATASET DOES NOT HAVE, PER ROW.\n"
        "                # #264 carried the scope column and nothing else, so every OTHER\n"
        "                # column the dataset omits was destroyed by the wholesale replace.\n"
        "                # Measured across r129/r130/r131: 12-14 columns per run even after\n"
        "                # name alignment -- users.email and users.password among them, which\n"
        "                # is why a seeded login form answered 401, and videos.created_at,\n"
        "                # which is why every timestamp read as None.\n"
        "                # Matched BY ID, never broadcast: copying one row's email to every\n"
        "                # row would collide on a unique column, which is the trap #264's own\n"
        "                # note describes. A dataset row with no id counterpart keeps only the\n"
        "                # scope default above.\n"
        "                # Match on id where both sides have one, else on a natural key. r131:\n"
        "                # videos agree on id (7 columns recovered), users carry no id on the\n"
        "                # lane side and comments carry none on the dataset side, so an\n"
        "                # id-only match recovered nothing for either. The keys below are\n"
        "                # generic identity columns, not this product's vocabulary.\n"
        "                _NAT_KEYS = ('id', 'username', 'email', 'slug', 'handle', 'name')\n"
        "                _by_id = {}\n"
        "                for _b in (_base if isinstance(_base, list) else []):\n"
        "                    if not isinstance(_b, dict):\n"
        "                        continue\n"
        "                    for _nk in _NAT_KEYS:\n"
        "                        _bv = _b.get(_nk)\n"
        "                        if isinstance(_bv, (str, int)) and _bv != '':\n"
        "                            _by_id.setdefault((_nk, _bv), _b)\n"
        "                _ds_keys = set()\n"
        "                for _r in _rows[:5]:\n"
        ...
        "                        for _nk in _NAT_KEYS:\n"
        "                            _mv = _m.get(_nk)\n"
        "                            if isinstance(_mv, (str, int)) and _mv != '':\n"
        "                                _src = _by_id.get((_nk, _mv))\n"
        "                                if _src is not None:\n"
        "                                    break\n"
        "                        if isinstance(_src, dict):\n"
        "                            for _k, _v in _src.items():\n"
        "                                if _k not in _ds_keys and _k not in _m:\n"
        "                                    _m[_k] = _v\n"
        "                        _out.append(_m)\n"
        "                    else:\n"
        "                        _out.append(_r)\n"
        ...
        "    finally:\n"
        "        db.close()\n\n\n"
        "def _live_parent_ids_1202u(db, fk_col, cache):\n"
        "    # Ids that ACTUALLY exist in the table `fk_col` points at, or [] when the column\n"
        "    # names no table we know. Cached per FK column: this runs per row.\n"
        "    key = str(fk_col)\n"
        "    if key in cache:\n"
        "        return cache[key]\n"
        "    out = []\n"
        ...
        "            pcls = getattr(models, _CLASS.get(cand, ''), None)\n"
        "            if pcls is None:\n"
        "                continue\n"
        "            pk = (_PK.get(cand) or ['id'])[0]\n"
        "            col = getattr(pcls, pk, None)\n"
        "            if col is None:\n"
        "                continue\n"
        "            out = [r[0] for r in db.query(col).all() if r[0] is not None]\n"
        "            break\n"
        "    except Exception:\n"
        "        out = []\n"
        "    cache[key] = out\n"
        ...
        "\n"
        "\n"
        "def _verify_seed_counts_1202r(db, data):\n"
        "    # #1202r: count what actually landed and SAY when it is short.\n"
        "    #\n"
        "    # Every insert here is wrapped in a SAVEPOINT (#218) so one bad row does not\n"
        "    # take the table down with it, and a row that still fails is dropped with only\n"
        "    # `_seed_dbg`, which prints nothing unless FW_DEBUG is set. The result is a\n"
        "    # silent shortfall, and it is not rare -- measured across the live delivered\n"
        "    # stacks of 13 runs:\n"
        "    #\n"
        ...
        "    #   r25              my_list 8->0\n"
        "    #\n"
        "    # The chain: some parent rows fail, so they get no #1112 pk-remap entry, so every\n"
        "    # child pointing at them violates its FK and is dropped too. r26 ships a Netflix\n"
        "    # whose Continue Watching rail can never fill.\n"
        "    #\n"
        "    # The framework has an audit for this and it has never run: its live row count\n"
        "    # succeeded 0 times in any run of the corpus, and its fallback filter examines 0\n"
        "    # tables in 145 of 147 runs. This check needs neither -- the session is open and\n"
        "    # the declared counts are right here.\n"
        "    #\n"
        "    # Reports; never raises, never blocks. A seeder that dies on its own bookkeeping\n"
        "    # would be worse than the silence it replaces.\n"
        "    try:\n"
        "        short = []\n"
        "        for t in _ORDER:\n"
        "            declared = len((data or {}).get(t) or [])\n"
        "            if not declared:\n"
        "                continue\n"
        "            cls = getattr(models, _CLASS.get(t, ''), None)\n"
        "            if cls is None:\n"
        "                continue\n"
        "            try:\n"
        "                actual = db.query(cls).count()\n"
        "            except Exception:\n"
        "                continue\n"
        "            if actual < declared:\n"
        "                short.append((t, declared, actual))\n"
        "        if short:\n"
        "            _sys.stderr.write(\n"
        "                'SEED SHORTFALL (#1202r): %d table(s) hold fewer rows than the seed '\n"
        "                'declares -- %s. Rows were dropped by their per-row SAVEPOINT, most '\n"
        "                'often a FK to a parent that itself failed. Set FW_DEBUG=1 to see the '\n"
        "                'per-row exceptions.\\n'\n"
        "                % (len(short), ', '.join('%s %d->%d' % r for r in short)))\n"
        "            _sys.stderr.flush()\n"
        "    except Exception:\n"
        "        pass\n"
        "\n"
        ...
        "                            row[_fkc] = _m[_fv]\n"
        "                            break\n"
        "                    # #1202u: the remap only knows parents that LANDED. When a\n"
        "                    # parent row failed, its children carry an id nothing has,\n"
        "                    # every one of them violates its FK, and the SAVEPOINT drops\n"
        "                    # them silently -- measured on delivered stacks: r26\n"
        "                    # continue_watching 8->1, my_list 8->1, ratings 8->1;\n"
        "                    # r30 genres 10->0, title_genres 15->0, my_list 22->2. A\n"
        "                    # Netflix whose Continue Watching can never fill, shipped.\n"
        "                    #\n"
        "                    # This is the policy the seeder ALREADY applies to an EMPTY\n"
        "                    # owner column (`_real_owner_ids`: point at ids that exist).\n"
        "                    # It was simply never applied to an FK the seed spelled out,\n"
        "                    # so an explicit `profile_id: 1` skipped it and died. A demo\n"
        "                    # row attached to a different real parent is worth more than\n"
        "                    # no row: only rows that were going to be DROPPED change.\n"
        "                    _live = _live_parent_ids_1202u(db, _fkc, _live_pid_cache)\n"
        "                    if _live and row.get(_fkc) not in _live:\n"
        "                        row[_fkc] = _live[i % len(_live)]\n"
        ...
        "        _verify_seed_counts_1202r(db, data)\n"
        "    finally:\n"
        "        db.close()\n"
