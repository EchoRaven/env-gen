...
### 350. the probe's stack survived my cleanup for 9 hours — and #962, the bug it exposed

**Correcting item 347 on two counts.**

1. I reported "probe project removed / generated/ back to exactly 154" and treated the cleanup as
   done. It was not. **A deleted directory is not a stopped stack.** Three containers
   (`docker_database_1`, `docker_backend_1`, `docker_frontend_1`, created 01:03:55), two built
   images (`localhost/docker_backend`, `localhost/docker_frontend`), two volumes and a network
   survived — still running at 09:54, nine hours later.
2. I said the probe reached `init -> implement` and implied it stopped there. It went further: it
   **built backend and frontend images and brought up a three-service stack**, under a model whose
   every reply is canned junk.

What that would have done to r155, had the key arrived first:

    ports      3001 / 8000 / 5433 were ALL held — exactly the ports the next run picks
    capture    the visual gate could have screenshotted the PROBE's frontend on :8000 and
               scored r155 with another app's pixels
    secrets    `docker_app_auth_keys` held a jwt_private.pem generated by the probe at 01:04,
               and r155 would have MOUNTED it

All removed; verified 0 containers / 0 volumes / 0 localhost images, three ports free, five base
images intact.

★ **#962 — the generalizable defect underneath.** The compose project name is derived from the
compose file's parent directory, which is always `<run>/docker/`. So **every run's project is named
`docker`**, containers are `docker_backend_1` with NO run identity, and named volumes/networks are
shared corpus-wide. That lands on code I added in #936: `container_id()`'s name-filter fallback did
`ids[0]`, so with two stacks up it answered about an ARBITRARY run's container — and its callers
are the staleness probes (#715/#738), i.e. precisely the checks whose job is to say whether the
served build is current.

#962 disambiguates on the `com.docker.compose.project.config_files` label and, when it still
cannot tell, returns `""`. **"Unknown" is recoverable; a confident wrong container is not** — and
callers already handle the empty case. Planted control: disabling the guard turns both ambiguity
tests red.

★★ The finding only exists because a stale stack was lying around, and the stale stack only existed
because my own cleanup was incomplete. **The mess was the instrument.** Had I cleaned up correctly
at 01:10, #962 would still be latent and r155 would have been the thing that found it — by scoring
itself against another app.

Left open (recorded, not guessed): giving each run its OWN compose project name would fix the class
